...
type P2PTransaction = {
    ...
    buyer_fields?: { key: string; label: string; value: string; locked?: boolean }[] | string | null;
    tx_id?: string | null;
...
const getTxBuyerFields = (tx: P2PTransaction) => {
    if (Array.isArray(tx.buyer_fields)) return tx.buyer_fields;
    if (!tx.buyer_fields) return [];
    try {
        const parsed = JSON.parse(String(tx.buyer_fields));
...
function BuyScreen({ onBack, walletBalance: initialWalletBalance, onGoRequest, onGoTopup, onGoSell }: { onBack: () => void; walletBalance: number; onGoRequest: () => void; onGoTopup: () => void; onGoSell: () => void }) {
    ...
    const [buyReceiveCurrency, setBuyReceiveCurrency] = useState<'USD' | 'USDT' | 'LKR'>('USD');
    ...
    const [sellPopupFieldVals, setSellPopupFieldVals] = useState<Record<string, string>>({});
        ...
        if (target) {
            ...
            if (!buySuccess && lockWasActive) {
                ...
                const body = endpoint === 'cancel'
                    ? JSON.stringify({ amount: buyAmount, receive_amount: buyReceiveAmount, receive_currency: buyReceiveCurrency })
                    : undefined;
                const unlockAfterLock = Promise.resolve(pendingBuyLockRef.current)
                    .catch(() => {})
                    .then(() => fetch(`${API_URL}/p2p-sell-ads/${target.id}/${endpoint}`, {
                        method: 'POST',
                        headers: {
    ...
    // Auto-sync the receive currency to whatever the seller set on their ad.
    useEffect(() => {
        if (!buyPopupAd) return;
        const adCur = (buyPopupAd.cryptoCurrency || (buyPopupAd.category === 'Crypto' ? 'USDT' : 'LKR')) as 'USD' | 'USDT' | 'LKR';
        if (adCur === 'USD' || adCur === 'USDT' || adCur === 'LKR') setBuyReceiveCurrency(adCur);
    }, [buyPopupAd?.id, buyPopupAd?.cryptoCurrency]);

    const buySellerCurrency = buyReceiveCurrency;
    const buyReceiveAmount = buyPopupAd && buyAmount
        ? (parseFloat(buyAmount) / Math.max(parseFloat(buyPopupAd.lkrRate || '1'), 0.0001)).toFixed(2)
        : '0.00';
    const getSellPopupAssignedFields = (ad: SavedPayment | null = buyPopupAd) => {
        if (!ad) return [];
        return adminMethods.find(method => method.name.toLowerCase() === ad.name.toLowerCase())?.fields || [];
    };
    const assignedSellPopupFields = getSellPopupAssignedFields();
    ...
    const getBuyAmountError = (value: string, ad: SavedPayment | null = buyPopupAd) => {
        ...
        const serverAvailable = Number(ad.availableAmount ?? ad.maxAmount);
        const localTx = localTransactionsByAdId[ad.id];
        const availableA = localTx?.status === 'pending' && localTx.reservesBalance
            ? Math.max(0, serverAvailable - Number(localTx.amount || 0))
            : serverAvailable;
        if (amt < minA) return `Minimum amount is R ${minA.toLocaleString()}.`;
        if (amt > maxA) return `Maximum amount is R ${maxA.toLocaleString()}.`;
        if (amt > availableA) return `Available balance is R ${availableA.toLocaleString()}.`;
        if (amt > walletBalance) return `Amount exceeds your wallet balance (R ${walletBalance.toFixed(2)}).`;
        return null;
    };
    const canSubmitSellPopup = !!buyPopupAd
        && !!buyAmount.trim()
        && !getBuyAmountError(buyAmount, buyPopupAd)
        && assignedSellPopupFields.every(f => String(sellPopupFieldVals[f.key] || '').trim());
    ...
    const handleBuyNow = async () => {
        ...
        const ad = buyPopupAd!;
        const emptyField = getSellPopupAssignedFields(ad).find(f => !String(sellPopupFieldVals[f.key] || '').trim());
        if (emptyField) { setBuyError(`"${emptyField.label}" is required.`); return; }
        const buyerFields = getSellPopupAssignedFields(ad).map(f => ({
            key: f.key,
            label: f.label,
            value: String(sellPopupFieldVals[f.key] || '').trim(),
            locked: f.locked,
        }));
        const localTx: P2PTransaction = {
            ...
            amount: buyAmount || '0',
            receive_amount: buyReceiveAmount,
            receive_currency: buyReceiveCurrency,
        ...
        setBuyStep('amount');
        try {
            const res = await fetch(`${API_URL}/p2p-sell-ads/${ad.id}/start`, {
                method: 'POST',
                headers: { Authorization: `Bearer ${getToken()}`, 'Content-Type': 'application/json' },
                body: JSON.stringify({ amount: buyAmount, receive_amount: buyReceiveAmount, receive_currency: buyReceiveCurrency, buyer_fields: buyerFields }),
            });
            const data = await res.json();
            if (!res.ok) {
    ...
    const fetchAds = (silent = false) => {
        ...
        authFetch(`${API_URL}/p2p-sell-ads`)
            .then(r => r.json())
            .then(data => {
                if (data.success) {
    ...
    const fetchTransactions = (_filter: TransactionFilter = transactionFilter) => {
        ...
        authFetch(`${API_URL}/p2p-sell-ads/transactions?status=all`)
            .then(r => r.json())
            .then(data => {
    ...
    useEffect(() => {
        ...
        if (token && typeof EventSource !== 'undefined') {
            ...
            const eventsUrl = `${API_URL}/p2p-sell-ads/events${separator}token=${encodeURIComponent(token)}`;
    ...
    useEffect(() => {
        ...
        const poll = setInterval(async () => {
            try {
                const res = await authFetch(`${API_URL}/p2p-sell-ads/transactions/${txId}`);
                const data = await res.json();
    ...
    const handleSave = async () => {
        ...
        const adminFields: { key: string; label: string; value: string; locked?: boolean }[] = [];
        if (formCategory === 'Bank' && country.trim()) {
            adminFields.push({ key: 'country', label: 'Country', value: country.trim(), locked: false });
        }
        ...
        setSaving(true);
        try {
            const url = editingId ? `${API_URL}/p2p-sell-ads/${editingId}` : `${API_URL}/p2p-sell-ads`;
    ...
    const handleDelete = async (id: string) => {
        ...
        setDeleting(true);
        try {
            const res = await fetch(`${API_URL}/p2p-sell-ads/${id}`, {
                method: 'DELETE',
                headers: { Authorization: `Bearer ${getToken()}` },
            });
    ...
    const getDisplayAvailableAmount = (payment: SavedPayment) => {
        ...
        if (localTx?.status === 'pending' && localTx.reservesBalance) {
            return Math.max(0, serverAvailable - Number(localTx.amount || 0));
        }
        return serverAvailable;
    };

    const hasSellerSubmittedProof = (tx: P2PTransaction) =>
        !!(tx.tx_id || tx.screenshot_data || tx.screenshot_name);
    ...
    const handleCancelPendingTransaction = async (tx: P2PTransaction, payment: SavedPayment) => {
        ...
        try {
            if (String(tx.id).startsWith('local-')) {
                await fetch(`${API_URL}/p2p-sell-ads/${payment.id}/cancel`, {
                    method: 'POST',
                    headers: { Authorization: `Bearer ${getToken()}`, 'Content-Type': 'application/json' },
                    body: JSON.stringify({ amount: tx.amount, receive_amount: tx.receive_amount }),
                });
            } else {
                await fetch(`${API_URL}/p2p-sell-ads/transactions/${tx.id}/cancel`, {
                    method: 'POST',
                    headers: { Authorization: `Bearer ${getToken()}` },
                });
            }
    ...
    const handleBuyerConfirmTransaction = async () => {
        if (!buyerViewPopup) return;
        setViewSubmitting(true);
        setViewError(null);
        try {
            const res = await fetch(`${API_URL}/p2p-sell-ads/transactions/${buyerViewPopup.tx.id}/confirm`, {
                method: 'POST',
                headers: { Authorization: `Bearer ${getToken()}` },
            });
            const data = await res.json();
    ...
    const handleConfirmPendingTransaction = async () => {
        ...
        try {
            if (!String(tx.id).startsWith('local-')) {
                await fetch(`${API_URL}/p2p-sell-ads/transactions/${tx.id}/confirm`, {
                    method: 'POST',
                    headers: { Authorization: `Bearer ${getToken()}` },
                });
            }
    ...
    return (
        <div className="pb-10 relative min-h-screen">
            <style jsx global>{`
                .googer-thin-scrollbar {
                    scrollbar-width: thin;
                    scrollbar-color: #3f3f46 #09090b;
                }
                .googer-thin-scrollbar::-webkit-scrollbar {
                    width: 6px;
                }
                .googer-thin-scrollbar::-webkit-scrollbar-track {
                    background: #09090b;
                }
                .googer-thin-scrollbar::-webkit-scrollbar-thumb {
                    background: #3f3f46;
                    border-radius: 999px;
                }
                .googer-thin-scrollbar::-webkit-scrollbar-thumb:hover {
                    background: #52525b;
                }
            `}</style>
            <div className="mb-3">
                    ...
                    {(() => {
                        ...
                        return (
                            <div
                                key={p.id}
                                onClick={() => {
                                    if (isOwn && transactionFilter === 'pending' && tx && txStatus === 'pending') openBuyerViewPopup(tx, p);
                                    else if (!isOwn && transactionFilter === 'pending' && tx && txStatus === 'pending') openBuyerViewPopup(tx, p);
                                }}
                                className={`bg-[#0d0d0d] border border-gray-800/70 rounded-2xl overflow-hidden transition-colors ${transactionFilter === 'pending' && tx && txStatus === 'pending' ? 'cursor-pointer hover:border-amber-500/40' : ''}`}
                            >
                                {/* Top section */}
                                            ...
                                            {isOwn && transactionFilter === 'pending' && tx && txStatus === 'pending' ? (
                                                ...
                                                transactionFilter === 'pending' && tx && txStatus === 'pending' ? (
                                                    ...
                                                    <button onClick={(e) => { e.stopPropagation(); openBuyPopup(p); }}
                                                        className="px-6 py-2 bg-red-500 hover:bg-red-400 text-white font-black rounded-xl text-xs tracking-wide transition-all active:scale-95 shadow-lg shadow-red-500/20">
                                                        Sell
                                                    </button>
                                                )
                                            )}
                        ...
                        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div className="bg-[#090909] border border-gray-800/70 rounded-xl px-4 py-3">
                                    <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest mb-1">You Pay</p>
                                    {buyerViewPopup.payment.isOwn ? (
                                        <p className="text-sm font-black text-white">
                                            {Number(buyerViewPopup.tx.receive_amount || 0).toLocaleString()}
                                            <span className="text-xs text-gray-500 ml-1">{buyerViewPopup.tx.receive_currency || buyerViewPopup.payment.cryptoCurrency || (buyerViewPopup.payment.category === 'Crypto' ? 'USDT' : 'LKR')}</span>
                                        </p>
                                    ) : (
                                        <p className="text-sm font-black text-white">{Number(buyerViewPopup.tx.amount || 0).toLocaleString()} <span className="text-xs text-gray-500">Rupieer</span></p>
                                    )}
                                </div>
                                <div className="bg-[#090909] border border-gray-800/70 rounded-xl px-4 py-3">
                                    <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest mb-1">You Receive</p>
                                    {buyerViewPopup.payment.isOwn ? (
                                        <p className="text-sm font-black text-emerald-400">{Number(buyerViewPopup.tx.amount || 0).toLocaleString()} <span className="text-xs text-gray-500">Rupieer</span></p>
                                    ) : (
                            ...
                            {/* Buyer filled payment method details */}
                            {getTxBuyerFields(buyerViewPopup.tx).length > 0 && (
                                <div className="bg-[#0a0a0a] border border-gray-800/50 rounded-xl px-4 py-3 space-y-2.5">
                                    <p className="text-[9px] font-black uppercase tracking-widest text-gray-600">Buyer Details</p>
                                    {getTxBuyerFields(buyerViewPopup.tx).map(f => {
                            ...
                            {hasSellerSubmittedProof(buyerViewPopup.tx) ? (
                                ...
                                <>
                                    ...
                                    <div>
                                        <label className="block text-[9px] font-bold text-gray-600 uppercase tracking-wider mb-1.5">Payment Screenshot <span className="text-red-400">*</span></label>
                        ...
                        {!hasSellerSubmittedProof(buyerViewPopup.tx) && buyerViewPopup.payment.isOwn && (
                            <div className="px-5 pb-5 pt-3 border-t border-gray-800 shrink-0">
                                <button onClick={handleBuyerSubmitProof} disabled={viewSubmitting}
                                    className="w-full py-2.5 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-60 text-black font-black rounded-xl text-[10px] uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-1.5">
                                    <IonIcon name="paper-plane-outline" className="text-xs" />
            ...
            {confirmTransaction && (
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
                    ...
                    <div className="relative w-full sm:max-w-md bg-[#0c0c0f] border border-gray-800 rounded-t-3xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[92vh]">
                        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-800 shrink-0">
                            <div>
                                <h2 className="text-xs font-bold text-white">Review Payment Request</h2>
                                <p className="text-[9px] text-gray-500">{confirmTransaction.payment.name} · Pending</p>
                            </div>
                        ...
                        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
                            ...
                            <div className="bg-[#090909] border border-gray-800/70 rounded-xl px-4 py-3">
                                <p className="text-[9px] text-gray-600 font-black uppercase tracking-widest mb-1">Transaction ID</p>
                                <p className="text-xs font-bold text-white break-all">{confirmTransaction.tx.tx_id || '-'}</p>
                            </div>
                            <div className="bg-[#090909] border border-gray-800/70 rounded-xl px-4 py-3">
                        ...
                        <div className="px-5 pb-5 pt-3 border-t border-gray-800 shrink-0 flex gap-2">
                            ...
                            <button onClick={handleConfirmPendingTransaction} disabled={confirmingTransaction} className="flex-1 py-2.5 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-60 text-black font-black rounded-xl text-[10px] uppercase tracking-widest transition-all active:scale-95">
            ...
            {buyPopupAd && buyStep === 'amount' && (
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
                    ...
                    <div className="relative w-full sm:max-w-sm bg-[#0c0c0f] border border-gray-800 rounded-t-3xl sm:rounded-2xl shadow-2xl flex flex-col">
                        ...
                        <div className="px-5 py-5 space-y-4 max-h-[76vh] overflow-y-auto googer-thin-scrollbar">
                            <div className="flex items-center justify-between bg-[#0a0a0a] border border-gray-800/50 rounded-xl px-4 py-3">
                                {(() => {
                                    ...
                                    return (
                                        <>
                                            <div>
                                                <p className="text-[9px] text-gray-600 uppercase tracking-wider font-bold mb-0.5">Wallet Balance</p>
                                                <p className="text-[11px] text-emerald-400 font-bold">Rupieer {walletBalance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-[9px] text-gray-600 uppercase tracking-wider font-bold mb-0.5">Limit</p>
                                                <p className="text-[11px] text-gray-300 font-semibold">Rupieer {minRupee.toLocaleString()} – Rupieer {maxRupee.toLocaleString()}</p>
                                                <p className="text-[9px] text-gray-600 uppercase tracking-wider font-bold mt-2 mb-0.5">Available</p>
                                                <p className="text-[11px] text-emerald-400 font-bold">Rupieer {liveAvailable.toLocaleString()}</p>
                                            </div>
                                        </>
                                    );
                                })()}
                            ...
                            <div>
                                <label className="block text-[9px] font-bold text-gray-600 uppercase tracking-wider mb-1.5">You Pay (Rupieer) <span className="text-red-400">*</span></label>
                                <div className="flex items-center bg-[#030303] border border-gray-700/50 rounded-lg overflow-hidden">
                                    <input type="number" value={buyAmount} onChange={e => handleBuyAmountChange(e.target.value)}
                                        placeholder={`Rupieer ${Number(buyPopupAd.minAmount).toLocaleString()} - Rupieer ${Number(buyPopupAd.maxAmount).toLocaleString()}`}
                                        autoFocus
                                        className="flex-1 bg-transparent px-3 py-2.5 text-sm font-bold text-white focus:outline-none min-w-0" />
                                    <span className="pr-3 text-[9px] font-black text-gray-500 shrink-0">Rupieer</span>
                                </div>
                                <p className="text-[9px] text-gray-700 mt-1">Cannot exceed your wallet balance.</p>
                            </div>
                            <div className="flex items-center justify-between bg-emerald-500/[0.06] border border-emerald-500/20 rounded-xl px-4 py-3">
                                <span className="text-[10px] text-gray-500 font-semibold">You Receive</span>
                                <div className="flex items-baseline gap-1">
                                    <span className="text-base font-black text-emerald-400">{buyReceiveAmount}</span>
                                    <span className="text-[10px] text-gray-500">{buyReceiveCurrency}</span>
                                </div>
                            </div>
                            {buyError && (
                                <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
                            ...
                            {(() => {
                                const assignedFields = assignedSellPopupFields;
                                if (assignedFields.length === 0) return null;
                                return (
                                    <div className="bg-[#0a0a0a] border border-gray-800/50 rounded-xl px-4 py-3">
                                        <p className="text-[9px] font-black uppercase tracking-widest text-gray-600 mb-2">Payment Method Fields</p>
                                        <div className="space-y-2.5 max-h-28 overflow-y-auto pr-1 googer-thin-scrollbar">
                                            {assignedFields.map(f => {
                                                return (
                                                    <div key={f.key}>
                                                        ...
                                                        <input
                                                            type={f.type || 'text'}
                                                            value={sellPopupFieldVals[f.key] || ''}
                                                            onChange={e => { setSellPopupFieldVals(prev => ({ ...prev, [f.key]: e.target.value })); setBuyError(null); }}
                                                            placeholder={f.placeholder || f.defaultValue || f.label}
                                                            className="w-full bg-[#030303] border border-gray-700/50 rounded-lg px-3 py-2 text-xs font-semibold text-white focus:outline-none focus:ring-1 focus:ring-white/20 hover:bg-[#0b0b0b] transition-all"
                                                        />
                                                    </div>
                                                );
                                            })}
                        ...
                        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-800 shrink-0">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-[9px] text-gray-500">Rupieer {buyAmount} → {buyReceiveCurrency} {buyReceiveAmount}</p>
                                </div>
                            </div>
                        </div>
                        {buySuccess ? (
                            ...
                            {buyBlockedTx && !buyBlockedTx.tx_id && (
                                <button
                                    onClick={async () => {
                                        ...
                                        try {
                                            await fetch(`${API_URL}/p2p-sell-ads/transactions/${buyBlockedTx.id}/cancel`, {
                                                method: 'POST',
                                                headers: { Authorization: `Bearer ${getToken()}` },
                                            });
...
export default function Topup() {
    ...
    const handlePayment = () => {
        if (paymentMethod === 'bank') {
            router.push(`/wallet/topup/bank-transfer?amount=${amount}`);
        } else {
                    ...
                    <button onClick={() => setStep('topup')} className="w-full text-left bg-[#070707] border border-gray-800 hover:border-gray-600 rounded-2xl p-6 transition-all active:scale-[0.98] group">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <p className="text-xs text-gray-500">Recharge your wallet with R</p>
                            </div>
                            <IonIcon name="chevron-forward-outline" className="text-gray-400 group-hover:text-white text-xl shrink-0 transition-colors" />
                        </div>
                    </button>
                    <button onClick={() => router.push('/wallet/request')} className="w-full text-left bg-[#070707] border border-gray-800 hover:border-gray-600 rounded-2xl p-6 transition-all active:scale-[0.98] group">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 border border-white/10 bg-black/20 rounded-xl flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
    ...
    if (step === 'buy') return (
        <BuyScreen
            ...
            onGoRequest={() => router.push('/wallet/request')}
            onGoTopup={() => setStep('topup')}
            onGoSell={() => router.push('/wallet/topup')}
        />
    );
