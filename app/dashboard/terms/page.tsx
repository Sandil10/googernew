...
Please read the Googer Privacy Policy carefully before using the platform.
...
https://googer.site/help-support`;
...
* Platform will deduct service fee (e.g. 10%â€“20%)
* Payments processed via approved payment methods
* Payouts may take X days (e.g. 3â€“7 working days)
...
By accepting this Agreement, you assert that you are at least 18 years old and fully capable of entering into and adhering to its terms.`;
...
function PolicyDetail({
    ...
    const eyebrow = activeTab === "privacy" ? "Privacy Policy" : "Terms & Conditions";

    return (
        <article className="rounded-[1.35rem] border border-white/8 bg-[#1f1f22] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)] sm:p-5">
            <p className="text-[10px] text-white/62">{eyebrow}</p>
            <div className="mt-4 space-y-4">
                {activeTab === "privacy"
                    ? (
                        <pre className="max-w-3xl whitespace-pre-wrap break-words font-sans text-[11px] leading-6 text-white/66 sm:text-[12px]">
                            {privacyContent}
                        </pre>
                    )
                    : (
...
export default function TermsPage() {
    ...
    const activeTitle = useMemo(() => "Terms and Policies", []);

    return (
        <div className="mx-auto max-w-5xl pb-10 text-white">
            <section className="overflow-hidden rounded-[2.2rem] border border-white/8 bg-[#1d1d1f] shadow-[0_30px_80px_rgba(0,0,0,0.45)]">
                <div className="h-2 w-full bg-gradient-to-r from-transparent via-[#f03131] to-transparent" />

                <div className="px-5 py-5 sm:px-7 sm:py-6">
                    <div className="flex items-start justify-between gap-4">
                        <div className="max-w-2xl">
                            <p className="text-[11px] font-black uppercase tracking-[0.45em] text-white/35">Googer Legal</p>
                            <h1 className="mt-3 text-[1.35rem] font-black tracking-tight text-white sm:text-[1.55rem]">{activeTitle}</h1>
                            <p className="mt-2 text-[11px] leading-6 text-white/60 sm:text-[12px]">
                                Please read the Googer {activeTab === "privacy" ? "Privacy Policy" : "Terms & Conditions"} carefully before using the platform.
                            </p>
                            <span className="mt-4 inline-flex rounded-full border border-white/10 bg-[#171719] px-3 py-2 text-[9px] font-black uppercase tracking-[0.28em] text-white/55">
                        ...
                        <button
                            type="button"
                            onClick={() => router.back()}
                            className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.03] px-3 py-2.5 text-[11px] font-bold text-white transition hover:bg-white/[0.07]"
                        >
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-2">
                        <button
                            ...
                            className={`rounded-[1rem] border px-3 py-3 text-left transition ${
                                activeTab === "terms"
                                    ? "border-white/[0.12] bg-[#232325] text-white"
                                    : "border-white/[0.08] bg-[#1a1a1d] text-white/88 hover:bg-[#202024]"
                            }`}
                        >
                            <p className="text-[9px] font-black uppercase tracking-[0.22em] text-white/35">Read</p>
                            <p className="mt-1.5 text-[13px] font-black tracking-tight sm:text-[14px]">Terms & Conditions</p>
                        </button>
                        ...
                        >
                            <p className={`text-[9px] font-black uppercase tracking-[0.22em] ${activeTab === "privacy" ? "text-black/45" : "text-white/35"}`}>Read</p>
                            <p className="mt-1.5 text-[13px] font-black tracking-tight sm:text-[14px]">Privacy Policy</p>
                        </button>
                    </div>
                </div>

                <div className="border-t border-white/8 bg-[#18181a] px-5 py-6 sm:px-7 sm:py-7">
                    <PolicyDetail activeTab={activeTab} />

                    <div className="mt-4 rounded-[1rem] border border-white/8 bg-[#1a1a1d] px-4 py-3 text-[11px] leading-5 text-white/60">
                        Need legal or privacy help? Open{" "}
                        <button
                            type="button"
