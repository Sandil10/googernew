...
export default function SettingsPage() {
    ...
    const [securityPanel] = useState<SecurityPanelKey>(null);
    ...
    const toggleTrustedDevice = async (device: AuthSessionDevice) => {
        try {
            ...
            setNotification({ type: "error", message: err?.message || "Could not update trusted device." });
        }
    };
    ...
    const logoutOtherDevices = async () => {
        try {
            ...
            setNotification({ type: "success", message: "Other logged devices were signed out." });
            await loadAuthSessions(true);
        } catch (err: any) {
            setNotification({ type: "error", message: err?.message || "Could not log out other devices." });
        }
    };
    ...
    const pendingSecurityDevices = useMemo(() => authSessions.filter((device) => device.approvalStatus === "pending"), [authSessions]);
    const loggedDevices = useMemo(() => authSessions.filter((device) => (
        device.isCurrent || (device.status === "active" && device.approvalStatus !== "pending")
    )), [authSessions]);
    const loginHistoryDevices = useMemo(() => [...authSessions].sort((a, b) => (
        new Date(b.loginAt || b.lastActiveAt || 0).getTime() - new Date(a.loginAt || a.lastActiveAt || 0).getTime()
    )), [authSessions]);
    const trustedDevicePageCount = Math.max(1, Math.ceil(loggedDevices.length / SECURITY_PAGE_SIZE));
    const loginHistoryPageCount = Math.max(1, Math.ceil(loginHistoryDevices.length / SECURITY_PAGE_SIZE));
    const pagedLoggedDevices = useMemo(() => {
        const start = (trustedDevicesPage - 1) * SECURITY_PAGE_SIZE;
        return loggedDevices.slice(start, start + SECURITY_PAGE_SIZE);
    }, [loggedDevices, trustedDevicesPage]);
    const pagedLoginHistoryDevices = useMemo(() => {
        const start = (loginHistoryPage - 1) * SECURITY_PAGE_SIZE;
        return loginHistoryDevices.slice(start, start + SECURITY_PAGE_SIZE);
    }, [loginHistoryDevices, loginHistoryPage]);

    useEffect(() => {
        setTrustedDevicesPage((page) => Math.min(page, trustedDevicePageCount));
    }, [trustedDevicePageCount]);

    useEffect(() => {
        setLoginHistoryPage((page) => Math.min(page, loginHistoryPageCount));
    }, [loginHistoryPageCount]);
                            ...
                            <section className="space-y-5">
                                <div className="rounded-3xl border border-white/8 bg-white/[0.03] p-5">
                                    ...
                                    <div className="grid gap-3 min-[860px]:grid-cols-2">
                                        {[
                                            ["changeEmail", "Change Login Email", "Verify the current login email by OTP before updating login email.", "shield-checkmark-outline", "/settings/change-login-email"],
                                            ["loginHistory", "Security Alerts", "Open Login History only, with 5 rows per page.", "notifications-outline", "/settings/security-alerts"],
                                            ["trustedDevices", "Trusted Devices", "Open logged devices only, with trusted controls and 5 rows per page.", "phone-portrait-outline", "/settings/trusted-devices"],
                                            ["passkey", "6-Digit Passkey", "Verify current email OTP before saving your account passkey.", "key-outline", "/settings/passkeys"],
                                            ["twoFactor", "2FA", "Add a phone number and choose email or phone for future OTPs.", "call-outline", "/settings/two-factor"],
                                            ["resetPassword", "Reset Password", "Verify current email OTP before creating a new password.", "mail-open-outline", "/settings/reset-password"],
                                        ].map(([key, title, subtitle, icon, href]) => (
                                            <Link
                                                key={title}
                                                href={href}
                                                className={`rounded-2xl border p-4 text-left transition ${securityPanel === key ? "border-white/25 bg-white/[0.08]" : "border-white/[0.08] bg-black/20 hover:border-white/15 hover:bg-white/[0.05]"}`}
                                            >
                                ...
                                {securityPanel === "trustedDevices" && (
                                <div className="rounded-3xl border border-white/8 bg-white/[0.03] p-5">
                                    <div className="mb-4 flex items-center justify-between gap-3">
                                        <div>
                                            <h2 className="text-base font-black text-white">Logged Devices</h2>
                                            <p className="mt-1 text-xs text-white/45">Current device first, trusted-device badges, new-device approval requests, and full device details.</p>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <button type="button" onClick={logoutOtherDevices} className="rounded-2xl bg-white px-3 py-2 text-[10px] font-bold uppercase tracking-[0.16em] text-black transition hover:bg-zinc-200">
                                                Log Out Others
                                            </button>
                                    ...
                                    {pendingSecurityDevices.map((device) => (
                                    ...
                                    <div className="grid gap-3">
                                                ...
                                                <div className="flex flex-col gap-4 min-[760px]:flex-row min-[760px]:items-center min-[760px]:justify-between">
                                                    ...
                                                    <div className="flex shrink-0 gap-2">
                                                        ...
                                                        {!device.isCurrent && (
                                                            <button
                                                                type="button"
                                                                onClick={() => toggleTrustedDevice(device)}
                                                                className={`rounded-2xl px-4 py-3 text-[10px] font-black uppercase tracking-[0.16em] transition ${device.trusted ? "border border-blue-400/25 bg-blue-500/10 text-blue-200 hover:bg-blue-500/15" : "border border-emerald-400/25 bg-emerald-500/10 text-emerald-200 hover:bg-emerald-500/15"}`}
                                                            >
                                        ...
                                        {!loadingSessions && loggedDevices.length === 0 && (
                                    ...
                                    {loggedDevices.length > SECURITY_PAGE_SIZE && (
                                        <div className="mt-4 flex items-center justify-between rounded-2xl border border-white/[0.08] bg-black/20 px-4 py-3">
                                            <span className="text-xs font-bold text-white/45">Page {trustedDevicesPage} of {trustedDevicePageCount}</span>
                                            <div className="flex gap-2">
                                                <button type="button" onClick={() => setTrustedDevicesPage((page) => Math.max(1, page - 1))} disabled={trustedDevicesPage <= 1} className="rounded-xl border border-white/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-white/60 transition hover:bg-white/5 disabled:opacity-40">Prev</button>
                                                <button type="button" onClick={() => setTrustedDevicesPage((page) => Math.min(trustedDevicePageCount, page + 1))} disabled={trustedDevicesPage >= trustedDevicePageCount} className="rounded-xl border border-white/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-white/60 transition hover:bg-white/5 disabled:opacity-40">Next</button>
                                            </div>
                                        </div>
                                    )}

                                    <div className="mt-4 rounded-3xl border border-white/[0.08] bg-black/20 p-4">
                                        <h3 className="text-sm font-black text-white">Trusted Devices</h3>
                                        <p className="mt-1 text-xs text-white/45">Devices you already trust stay easier to recognize when new approval requests arrive.</p>
                                        <div className="mt-3 flex flex-wrap gap-2">
                                            {authSessions.filter((device) => device.trusted && device.status === "active").map((device) => (
                                                <button
                                                    key={`trusted-${device.id}`}
                                                    type="button"
                                                    onClick={() => setSelectedDevice(device)}
                                                    className="rounded-full border border-emerald-400/25 bg-emerald-400/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.14em] text-emerald-200 transition hover:bg-emerald-400/15"
                                                >
                                                    {device.isCurrent ? "Current Device" : device.deviceName}
                                                </button>
                                            ))}
                                            {authSessions.filter((device) => device.trusted && device.status === "active").length === 0 && (
                                                <span className="text-xs text-white/40">No trusted devices yet.</span>
                                            )}
                                        </div>
                                    </div>
                                </div>
                                ...
                                {securityPanel === "loginHistory" && (
                                    <div className="rounded-3xl border border-white/8 bg-white/[0.03] p-5">
                                        <div className="mb-4 flex items-center justify-between gap-3">
                                            <div>
                                                <h2 className="text-base font-black text-white">Login History</h2>
                                                <p className="mt-1 text-xs text-white/45">Security Alerts opens this table only. Each page shows 5 login rows.</p>
                                            </div>
                                            <button type="button" onClick={() => loadAuthSessions()} className="rounded-2xl border border-white/10 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.16em] text-white/60 transition hover:bg-white/5 hover:text-white">
                                                {loadingSessions ? "Loading" : "Refresh"}
                                            </button>
                                        ...
                                        <div className="overflow-hidden rounded-3xl border border-white/[0.08] bg-black/20">
                                            <div className="grid grid-cols-[1.4fr_1fr_1fr_.8fr] gap-3 border-b border-white/[0.08] px-4 py-3 text-[10px] font-black uppercase tracking-[0.16em] text-white/35">
                                                <span>Device</span>
                                                <span>Location</span>
                                                <span>Login Time</span>
                                                <span>Status</span>
                                            </div>
                                            {pagedLoginHistoryDevices.map((device) => (
                                                <button key={`history-${device.id}`} type="button" onClick={() => setSelectedDevice(device)} className="grid w-full grid-cols-[1.4fr_1fr_1fr_.8fr] gap-3 border-b border-white/[0.06] px-4 py-3 text-left text-xs transition last:border-b-0 hover:bg-white/[0.04]">
                                                    <span className="min-w-0">
                                                        <span className="block truncate font-black text-white">{device.deviceName}</span>
                                                        <span className="mt-1 block truncate text-white/40">{device.browser} on {device.operatingSystem}</span>
                                                    </span>
                                                    <span className="truncate text-white/55">{formatDeviceLocation(device)}</span>
                                                    <span className="truncate text-white/55">{formatDeviceDate(device.loginAt)}</span>
                                                    <span className="truncate text-white/55">{device.loginResult || device.status || "success"}</span>
                                                </button>
                                            ))}
                                            {!loadingSessions && loginHistoryDevices.length === 0 && (
                                                <div className="px-4 py-8 text-center text-xs text-white/45">No login history found yet.</div>
                                            )}
                                        </div>

                                        {loginHistoryDevices.length > SECURITY_PAGE_SIZE && (
                                            <div className="mt-4 flex items-center justify-between rounded-2xl border border-white/[0.08] bg-black/20 px-4 py-3">
                                                <span className="text-xs font-bold text-white/45">Page {loginHistoryPage} of {loginHistoryPageCount}</span>
                                                <div className="flex gap-2">
                                                    <button type="button" onClick={() => setLoginHistoryPage((page) => Math.max(1, page - 1))} disabled={loginHistoryPage <= 1} className="rounded-xl border border-white/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-white/60 transition hover:bg-white/5 disabled:opacity-40">Prev</button>
                                                    <button type="button" onClick={() => setLoginHistoryPage((page) => Math.min(loginHistoryPageCount, page + 1))} disabled={loginHistoryPage >= loginHistoryPageCount} className="rounded-xl border border-white/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.16em] text-white/60 transition hover:bg-white/5 disabled:opacity-40">Next</button>
                                                </div>
                                            </div>
                                        )}
                                    </div>
