...
const { getMediaStorageConfig, getMediaStorageProvider } = require('./config');
...
if (useCloudinary) {
    ...
    cloudinary.config({
        cloud_name: mediaStorageConfig.cloudinary.cloudName,
        api_key: mediaStorageConfig.cloudinary.apiKey,
...
const saveUploadedFile = async (file, folder = 'media') => {
    ...
    const uploadDir = path.join(mediaStorageConfig.publicUploadRoot, safeFolder);
