...
const p2pSellAdEvents = global.__p2pSellAdEvents || new EventEmitter();
global.__p2pSellAdEvents = p2pSellAdEvents;
p2pSellAdEvents.setMaxListeners(0);
...
const emitAdLockChange = (type, adId, locked) => {
    p2pSellAdEvents.emit('ad-lock-change', {
        type,
        adId: String(adId),
        locked: !!locked,
        timestamp: Date.now(),
...
const unlockBuyerForAd = async (adId, buyerId, type = 'unlock') => {
    await pool.query(
        'DELETE FROM p2p_sell_active_buyers WHERE ad_id=$1 AND buyer_id=$2',
        [adId, buyerId]
    );
    const lockCheck = await pool.query('SELECT COUNT(*) FROM p2p_sell_active_buyers WHERE ad_id=$1', [adId]);
...
const ensureTable = async () => {
    await pool.query(`
        CREATE TABLE IF NOT EXISTS p2p_sell_ads (
    ...
    `);
    await pool.query(`ALTER TABLE p2p_sell_ads ADD COLUMN IF NOT EXISTS description TEXT`).catch(() => {});
    await pool.query(`
        CREATE TABLE IF NOT EXISTS p2p_sell_active_buyers (
            ad_id      INTEGER NOT NULL REFERENCES p2p_sell_ads(id) ON DELETE CASCADE,
    ...
    `);
    await pool.query(`
        CREATE TABLE IF NOT EXISTS p2p_sell_transactions (
            id               SERIAL PRIMARY KEY,
            ad_id            INTEGER NOT NULL REFERENCES p2p_sell_ads(id) ON DELETE CASCADE,
    ...
    `);
    await pool.query(`ALTER TABLE p2p_sell_transactions ADD COLUMN IF NOT EXISTS screenshot_data TEXT`);
    await pool.query(`ALTER TABLE p2p_sell_transactions ADD COLUMN IF NOT EXISTS screenshot_name TEXT`);
    await pool.query(`ALTER TABLE p2p_sell_transactions ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ`);
    await pool.query(`ALTER TABLE p2p_sell_transactions ADD COLUMN IF NOT EXISTS receive_currency TEXT`);
    await pool.query(`ALTER TABLE p2p_sell_transactions ADD COLUMN IF NOT EXISTS buyer_fields JSONB NOT NULL DEFAULT '[]'`);
    await pool.query(`ALTER TABLE p2p_sell_transactions ADD COLUMN IF NOT EXISTS buyer_report_reason TEXT`);
    await pool.query(`ALTER TABLE p2p_sell_transactions ADD COLUMN IF NOT EXISTS buyer_reported_at TIMESTAMPTZ`);
    await pool.query(`ALTER TABLE p2p_sell_transactions ADD COLUMN IF NOT EXISTS seller_report_reason TEXT`);
    await pool.query(`ALTER TABLE p2p_sell_transactions ADD COLUMN IF NOT EXISTS seller_reported_at TIMESTAMPTZ`);
    // Disable any database-level auto-cancel triggers so transactions only cancel on explicit user action
    await pool.query(`
        DO $$ BEGIN
            DROP TRIGGER IF EXISTS auto_cancel_pending ON p2p_sell_transactions;
            DROP TRIGGER IF EXISTS cancel_expired_transactions ON p2p_sell_transactions;
            DROP TRIGGER IF EXISTS expire_pending_transactions ON p2p_sell_transactions;
            DROP TRIGGER IF EXISTS auto_expire_p2p_sell_transactions ON p2p_sell_transactions;
        END $$;
    `).catch(() => {});
    await pool.query(`
        DO $$ BEGIN
            PERFORM cron.unschedule(jobname) FROM cron.job
            WHERE jobname ILIKE '%p2p%sell%cancel%' OR jobname ILIKE '%p2p%sell%expire%'
               OR command ILIKE '%p2p_sell_transactions%cancel%';
        END $$;
    `).catch(() => {});
};
...
router.get('/events', async (req, res) => {
    ...
    p2pSellAdEvents.on('ad-lock-change', send);
    req.on('close', () => {
        clearInterval(keepAlive);
        p2pSellAdEvents.off('ad-lock-change', send);
        res.end();
    });
});
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        await pool.query(`DELETE FROM p2p_sell_active_buyers WHERE created_at < NOW() - INTERVAL '25 seconds'`);
        const result = await pool.query(`
            SELECT a.*,
                   u.username, u.profile_picture, u.full_name,
                   (a.user_id = $1)          AS is_own,
                   EXISTS(
                       SELECT 1 FROM p2p_sell_transactions t
            ...
            FROM p2p_sell_ads a
            JOIN users u ON u.id = a.user_id
            LEFT JOIN p2p_sell_active_buyers ab ON ab.ad_id = a.id
            WHERE a.user_id = $1
               OR u.wallet_balance >= ${AVAILABLE_AMOUNT_SQL}
               OR EXISTS (
                   SELECT 1 FROM p2p_sell_transactions mine
        ...
        console.error('GET /p2p-sell-ads error:', err);
...
router.get('/transactions', authMiddleware, async (req, res) => {
    try {
        ...
        console.error('GET /p2p-sell-ads/transactions error:', err);
...
router.get('/transactions/:transactionId', authMiddleware, async (req, res) => {
    try {
        ...
        console.error('GET /p2p-sell-ads/transactions/:id error:', err);
...
router.post('/transactions/:transactionId/report', authMiddleware, async (req, res) => {
    try {
        ...
        const txRow = await pool.query(
            `SELECT * FROM p2p_sell_transactions
             WHERE id=$1
        ...
        const result = await pool.query(
            `UPDATE p2p_sell_transactions SET ${reasonColumn}=$1, ${timeColumn}=NOW() WHERE id=$2 RETURNING *`,
            [reason, transactionId]
        );

        res.json({ success: true, transaction: result.rows[0] });
    } catch (err) {
        console.error('POST /p2p-sell-ads/transactions/:transactionId/report error:', err);
...
router.post('/transactions/:transactionId/cancel', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const txRow = await pool.query(
            'SELECT * FROM p2p_sell_transactions WHERE id=$1 AND (buyer_id=$2 OR seller_id=$2) AND status=$3',
        ...
        const tx = txRow.rows[0];
        const holdAmount = parseFloat(tx.amount) || 0;
        ...
        await client.query(
            'UPDATE p2p_sell_transactions SET status=$1 WHERE id=$2',
        ...
        const lockCheck = await pool.query('SELECT COUNT(*) FROM p2p_sell_active_buyers WHERE ad_id=$1', [tx.ad_id]);
        emitAdLockChange('transaction-cancel', tx.ad_id, parseInt(lockCheck.rows[0].count, 10) > 0);
        res.json({ success: true });
    } catch (err) {
        await client.query('ROLLBACK').catch(() => {});
        console.error('POST /p2p-sell-ads/transactions/:transactionId/cancel error:', err);
...
router.post('/transactions/:transactionId/confirm', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const txRow = await pool.query(
            'SELECT * FROM p2p_sell_transactions WHERE id=$1 AND buyer_id=$2 AND status=$3',
            [transactionId, userId, 'pending']
        );
        if (!txRow.rows.length) {
            return res.status(404).json({ success: false, message: 'Pending transaction not found.' });
        ...
        const txResult = await client.query(
            'UPDATE p2p_sell_transactions SET status=$1, completed_at=NOW() WHERE id=$2 RETURNING *',
        ...
        await client.query('ROLLBACK').catch(() => {});
        console.error('POST /p2p-sell-ads/transactions/:transactionId/confirm error:', err);
...
// POST buyer starts pending hold. For sell flow:
// - The buyer's `amount` is denominated in R (their wallet balance) and is capped client-side at the buyer's wallet_balance.
// - `receive_amount` represents the foreign-currency amount they will receive in the buyer-chosen currency.
// - `receive_currency` is the buyer-chosen receive currency (USD / USDT / LKR).
router.post('/:id/start', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const { amount, receive_amount, receive_currency, buyer_fields } = req.body;
        ...
        const validation = validateRequestedAmount(adRow.rows[0], amount);
        ...
        const buyerFields = parseJsonArray(buyer_fields);

        await client.query('BEGIN');

        // Sell flow: hold from BUYER's wallet (the buyer is spending R to receive foreign currency).
        const buyerRow = await client.query(
            'SELECT wallet_balance FROM users WHERE id=$1 FOR UPDATE',
            [buyerId]
        );
        const buyerBalance = parseFloat(buyerRow.rows[0]?.wallet_balance || 0);
        if (requestedAmount > buyerBalance) {
        ...
        if (requestedAmount > sellerBalance) {
            await client.query('ROLLBACK');
        ...
        if (requestedAmount > 0) {
            await client.query(
                ...
                [requestedAmount, buyerId]
            );
        }

        const txResult = await client.query(`
            INSERT INTO p2p_sell_transactions (ad_id, buyer_id, seller_id, amount, receive_amount, receive_currency, buyer_fields, tx_id, status)
            VALUES ($1, $2, $3, $4, $5, $6, $7, NULL, 'pending')
            RETURNING *
        `, [adId, buyerId, adRow.rows[0].user_id, requestedAmount, receive_amount || null, receive_currency || null, JSON.stringify(buyerFields)]);
        ...
        await client.query('ROLLBACK').catch(() => {});
        console.error('POST /p2p-sell-ads/:id/start error:', err);
...
router.post('/transactions/:transactionId/submit-details', authMiddleware, upload.single('screenshot'), async (req, res) => {
    try {
        ...
        if (req.file) {
            screenshotUrl = await saveUploadedFile(req.file, 'p2p-sell-proofs');
            screenshotName = req.file.originalname || req.file.filename || null;
        }

        const txResult = await pool.query(`
            UPDATE p2p_sell_transactions
            SET tx_id=$1,
                screenshot_data=$2,
                screenshot_name=$3
            WHERE id=$4
        ...
        console.error('POST /p2p-sell-ads/transactions/:transactionId/submit-details error:', err);
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const existing = await pool.query(
            'SELECT id FROM p2p_sell_ads WHERE user_id = $1 AND catalog_id = $2 LIMIT 1',
            [userId, catalog_id]
        );
        if (existing.rows.length > 0) {
            return res.status(409).json({ success: false, message: 'You already have a sell ad for this payment method. Delete it before posting another.' });
        }

        const result = await pool.query(`
            INSERT INTO p2p_sell_ads
        ...
        console.error('POST /p2p-sell-ads error:', err);
...
router.put('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const check = await pool.query('SELECT user_id FROM p2p_sell_ads WHERE id=$1', [adId]);
        ...
        const lockCheck = await pool.query(`
            SELECT COUNT(*)::int AS count FROM p2p_sell_transactions WHERE ad_id=$1 AND status='pending'
        `, [adId]);
        ...
        console.error('PUT /p2p-sell-ads/:id error:', err);
...
router.delete('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        await pool.query('DELETE FROM p2p_sell_active_buyers WHERE ad_id=$1', [adId]);
        await pool.query('DELETE FROM p2p_sell_ads WHERE id=$1', [adId]);
        res.json({ success: true });
    } catch (err) {
        console.error('DELETE /p2p-sell-ads/:id error:', err);
...
router.post('/:id/force-unlock', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const pendings = await pool.query(
            `SELECT id, buyer_id, amount FROM p2p_sell_transactions WHERE ad_id=$1 AND status='pending'`,
            [adId]
        );

        await client.query('BEGIN');
        for (const row of pendings.rows) {
            const hold = parseFloat(row.amount) || 0;
            if (hold > 0) {
                await client.query(
                    'UPDATE users SET hold_balance = GREATEST(hold_balance - $1, 0), wallet_balance = wallet_balance + $1 WHERE id = $2',
                    [hold, row.buyer_id]
            ...
            await client.query("UPDATE p2p_sell_transactions SET status='cancelled' WHERE id=$1", [row.id]);
        }
        await client.query('DELETE FROM p2p_sell_active_buyers WHERE ad_id=$1', [adId]);
        await client.query('COMMIT');
        ...
        await client.query('ROLLBACK').catch(() => {});
        console.error('POST /p2p-sell-ads/:id/force-unlock error:', err);
...
router.post('/:id/lock', authMiddleware, async (req, res) => {
    try {
        ...
        await pool.query(`
            INSERT INTO p2p_sell_active_buyers (ad_id, buyer_id) VALUES ($1, $2)
        ...
        console.error('POST /p2p-sell-ads/:id/lock error:', err);
...
router.post('/:id/unlock', authMiddleware, async (req, res) => {
    try {
        ...
        console.error('POST /p2p-sell-ads/:id/unlock error:', err);
...
router.post('/:id/cancel', authMiddleware, async (req, res) => {
    try {
        ...
        const buyerId = resolveUserId(req);
        const adId = parseInt(req.params.id, 10);
        const { amount, receive_amount, receive_currency } = req.body || {};

        const adRow = await pool.query('SELECT user_id FROM p2p_sell_ads WHERE id=$1', [adId]);
        if (!adRow.rows.length) return res.status(404).json({ success: false, message: 'Ad not found.' });

        await pool.query(`
            INSERT INTO p2p_sell_transactions (ad_id, buyer_id, seller_id, amount, receive_amount, receive_currency, tx_id, status)
            VALUES ($1, $2, $3, $4, $5, $6, NULL, 'cancelled')
        `, [adId, buyerId, adRow.rows[0].user_id, parseFloat(amount) || 0, receive_amount || null, receive_currency || null]);

        await unlockBuyerForAd(adId, buyerId, 'cancel');
        res.json({ success: true });
    } catch (err) {
        console.error('POST /p2p-sell-ads/:id/cancel error:', err);
...
router.post('/:id/complete', authMiddleware, async (req, res) => {
    try {
        ...
        const buyerId = resolveUserId(req);
        const adId = parseInt(req.params.id, 10);
        const { amount, receive_amount, receive_currency, tx_id, screenshot_data, screenshot_name } = req.body;
        ...
        const txResult = await pool.query(`
            INSERT INTO p2p_sell_transactions (ad_id, buyer_id, seller_id, amount, receive_amount, receive_currency, tx_id, screenshot_data, screenshot_name, status)
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'pending')
            RETURNING *
        `, [adId, buyerId, adRow.rows[0].user_id, validation.requestedAmount, receive_amount || null, receive_currency || null, tx_id || null, screenshot_data || null, screenshot_name || null]);

        await unlockBuyerForAd(adId, buyerId, 'complete');

        res.json({ success: true, transaction: txResult.rows[0] });
    } catch (err) {
        console.error('POST /p2p-sell-ads/:id/complete error:', err);
