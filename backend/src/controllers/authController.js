...
let accountSecurityOtpTableEnsured = false;
...
const ensureAccountSecuritySchema = async () => {
    ...
    await pool.query(`
        CREATE TABLE IF NOT EXISTS account_security_otps (
            ...
            security_token_hash VARCHAR(128),
    ...
    `);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_account_security_otps_user_purpose ON account_security_otps(user_id, purpose, created_at DESC)`);
    ...
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS two_factor_phone_country_code VARCHAR(10)`);
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS two_factor_phone_country_name VARCHAR(120)`);
    await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS two_factor_phone_dial_code VARCHAR(12)`);
...
exports.getAuthSessionHistory = async (req, res) => {
    try {
        await ensureAuthSessionsTable();
        const result = await pool.query(
            `SELECT *
             FROM auth_sessions
             WHERE user_id = $1
             ORDER BY COALESCE(last_active_at, login_at, created_at) DESC
             LIMIT 120`,
            [req.user.id]
        );
        const currentSessionId = req.user.sessionId || null;
        res.json({
            success: true,
            history: result.rows.map((row) => mapAuthSession(row, currentSessionId)),
        });
    } catch (error) {
        console.error('Get auth session history error:', error);
        res.status(500).json({ success: false, message: 'Could not load login history' });
    }
};

exports.logoutOtherAuthSessions = async (req, res) => {
    try {
        await ensureAuthSessionsTable();
        const currentSessionId = req.user.sessionId || null;
        const result = await pool.query(
        ...
        res.json({
            success: true,
            loggedOutCount: result.rowCount || 0,
        });
    } catch (error) {
        console.error('Logout other auth sessions error:', error);
        res.status(500).json({ success: false, message: 'Could not log out other devices' });
    }
};
...
exports.requestPasswordResetOtp = async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            ...(process.env.PASSWORD_RESET_DEBUG_OTP === 'true' ? { debugOtp: otp } : {}),
        });
    } catch (error) {
...
const normalizeSecurityPurpose = (purpose) => String(purpose || '').trim();

const getSecurityUser = async (userId) => {
    await ensureAccountSecuritySchema();
    const result = await pool.query(
        `SELECT id, email, full_name, username, two_factor_phone_country_code,
                two_factor_phone_country_name, two_factor_phone_dial_code,
                two_factor_phone_number, otp_delivery_method
         FROM users
         WHERE id = $1
         LIMIT 1`,
        [userId]
...
const consumeSecurityToken = async (client, userId, purpose, securityToken) => {
    const tokenHash = hashResetValue(securityToken);
    const result = await client.query(
        `SELECT id
         FROM account_security_otps
         WHERE user_id = $1
           AND purpose = $2
           AND security_token_hash = $3
           AND verified_at IS NOT NULL
           AND used_at IS NULL
           AND expires_at > NOW()
         ORDER BY verified_at DESC
    ...
    await client.query('UPDATE account_security_otps SET used_at = NOW() WHERE id = $1', [result.rows[0].id]);
};

exports.requestAccountSecurityOtp = async (req, res) => {
    try {
        const purpose = normalizeSecurityPurpose(req.body?.purpose);
        const destinationType = String(req.body?.destinationType || 'email').trim() === 'phone' ? 'phone' : 'email';
        if (!SECURITY_OTP_PURPOSES.has(purpose)) {
            return res.status(400).json({ success: false, message: 'Valid security purpose is required.' });
        }

        const user = await getSecurityUser(req.user.id);
        if (!user) return res.status(404).json({ success: false, message: 'User not found' });

        let destination = user.email;
        if (destinationType === 'phone') {
            const incomingPhone = String(req.body?.phoneNumber || '').replace(/[^\d]/g, '');
            const incomingDial = String(req.body?.dialCode || '').replace(/[^\d+]/g, '');
            destination = incomingPhone && incomingDial
                ? `${incomingDial}${incomingPhone.replace(/^0+/, '')}`
                : `${user.two_factor_phone_dial_code || ''}${user.two_factor_phone_number || ''}`;
        }
        if (!destination) {
            return res.status(400).json({ success: false, message: destinationType === 'phone' ? 'Phone number is required.' : 'Email is required.' });
        }
        ...
        await pool.query(
            `INSERT INTO account_security_otps
                (user_id, purpose, destination_type, destination, otp_hash, expires_at, requested_ip, user_agent)
             VALUES ($1, $2, $3, $4, $5, NOW() + ($6::text || ' minutes')::interval, $7, $8)`,
            [req.user.id, purpose, destinationType, destination, hashResetValue(otp), SECURITY_OTP_TTL_MINUTES, req.ip || null, req.get('user-agent') || null]
        );
        ...
        return res.status(200).json({
            success: true,
            message: destinationType === 'phone' ? 'Debug phone OTP generated.' : 'OTP sent to current email.',
            destinationType,
            maskedDestination: destinationType === 'phone' ? destination.replace(/\d(?=\d{3})/g, '*') : destination,
            expiresInMinutes: SECURITY_OTP_TTL_MINUTES,
            debugOtp: otp,
        });
    } catch (error) {
        console.error('Request account security OTP error:', error);
        return res.status(500).json({ success: false, message: 'Could not send security OTP.' });
    }
};

exports.verifyAccountSecurityOtp = async (req, res) => {
    try {
        const purpose = normalizeSecurityPurpose(req.body?.purpose);
        const otp = String(req.body?.otp || '').trim();
        if (!SECURITY_OTP_PURPOSES.has(purpose) || !/^\d{6}$/.test(otp)) {
            return res.status(400).json({ success: false, message: 'Valid purpose and 6-digit OTP are required.' });
        }
        ...
        const result = await pool.query(
            `SELECT id
             ...
             ORDER BY created_at DESC
             LIMIT 1`,
            [req.user.id, purpose, hashResetValue(otp)]
        );
        if (result.rows.length === 0) {
            return res.status(400).json({ success: false, message: 'Invalid or expired OTP.' });
        }
        ...
        await pool.query(
            `UPDATE account_security_otps
             SET verified_at = NOW(), security_token_hash = $1
             WHERE id = $2`,
            [hashResetValue(securityToken), result.rows[0].id]
        );

        return res.status(200).json({ success: true, message: 'OTP verified.', securityToken });
    } catch (error) {
        console.error('Verify account security OTP error:', error);
        return res.status(500).json({ success: false, message: 'Could not verify security OTP.' });
    }
};

exports.changeLoginEmailWithOtp = async (req, res) => {
    const client = await pool.connect();
    try {
        const newEmail = normalizeResetEmail(req.body?.newEmail);
        const securityToken = String(req.body?.securityToken || '').trim();
        if (!newEmail || !securityToken) {
            return res.status(400).json({ success: false, message: 'New email and verified OTP token are required.' });
        }

        await ensureAccountSecuritySchema();
        await client.query('BEGIN');
        await consumeSecurityToken(client, req.user.id, 'change_email', securityToken);
        const conflict = await client.query('SELECT id FROM users WHERE LOWER(email) = LOWER($1) AND id <> $2 LIMIT 1', [newEmail, req.user.id]);
        if (conflict.rows.length) {
            await client.query('ROLLBACK');
            return res.status(409).json({ success: false, message: 'This email is already used by another account.' });
        }
        const updated = await client.query(
            `UPDATE users SET email = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING id, user_id, username, full_name, email, contact_email, phone_number`,
            [newEmail, req.user.id]
        );
        await client.query('COMMIT');
        return res.status(200).json({ success: true, message: 'Login email updated.', user: updated.rows[0] });
    } catch (error) {
        await client.query('ROLLBACK').catch(() => {});
        console.error('Change login email with OTP error:', error);
        return res.status(error.statusCode || 500).json({ success: false, message: error.message || 'Could not update login email.' });
    } finally {
        client.release();
    }
};

exports.resetLoggedInPasswordWithOtp = async (req, res) => {
    const client = await pool.connect();
    try {
        const securityToken = String(req.body?.securityToken || '').trim();
        const newPassword = String(req.body?.newPassword || '');
        const passwordValidation = validatePassword(newPassword);
        if (!securityToken || !passwordValidation.valid) {
            return res.status(400).json({ success: false, message: passwordValidation.message || 'Verified OTP token is required.' });
        }

        await ensureAccountSecuritySchema();
        await client.query('BEGIN');
        await consumeSecurityToken(client, req.user.id, 'reset_password', securityToken);
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(newPassword, salt);
        await client.query('UPDATE users SET password = $1, token_version = COALESCE(token_version, 0) + 1 WHERE id = $2', [hashedPassword, req.user.id]);
        await client.query('COMMIT');
        return res.status(200).json({ success: true, message: 'Password reset successfully.' });
    } catch (error) {
        await client.query('ROLLBACK').catch(() => {});
        console.error('Logged-in reset password with OTP error:', error);
        return res.status(error.statusCode || 500).json({ success: false, message: error.message || 'Could not reset password.' });
    } finally {
        client.release();
    }
};

exports.savePasskeyWithOtp = async (req, res) => {
    const client = await pool.connect();
    try {
        const securityToken = String(req.body?.securityToken || '').trim();
        const passkey = String(req.body?.passkey || '').trim();
        if (!securityToken || !/^\d{6}$/.test(passkey)) {
            return res.status(400).json({ success: false, message: 'Verified OTP token and 6-digit passkey are required.' });
        }
        await ensureAccountSecuritySchema();
        await client.query('BEGIN');
        await consumeSecurityToken(client, req.user.id, 'passkey', securityToken);
        const salt = await bcrypt.genSalt(10);
        const passkeyHash = await bcrypt.hash(passkey, salt);
        await client.query('UPDATE users SET passkey_hash = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2', [passkeyHash, req.user.id]);
        await client.query('COMMIT');
        return res.status(200).json({ success: true, message: '6-digit passkey saved.' });
    } catch (error) {
        await client.query('ROLLBACK').catch(() => {});
        console.error('Save passkey with OTP error:', error);
        return res.status(error.statusCode || 500).json({ success: false, message: error.message || 'Could not save passkey.' });
    } finally {
        client.release();
    }
};

exports.saveTwoFactorPhone = async (req, res) => {
    const client = await pool.connect();
    try {
        const emailSecurityToken = String(req.body?.emailSecurityToken || '').trim();
        const phoneSecurityToken = String(req.body?.phoneSecurityToken || '').trim();
        ...
        const updated = await client.query(
            `UPDATE users
             ...
             RETURNING id, email, two_factor_enabled, two_factor_phone_country_code, two_factor_phone_country_name,
                       two_factor_phone_dial_code, two_factor_phone_number, otp_delivery_method`,
            [countryCode, countryName, dialCode, phoneNumber, otpDeliveryMethod, req.user.id]
        );
        await client.query('COMMIT');
        return res.status(200).json({ success: true, message: '2FA phone saved.', user: updated.rows[0] });
    } catch (error) {
        await client.query('ROLLBACK').catch(() => {});
        console.error('Save two-factor phone error:', error);
        return res.status(error.statusCode || 500).json({ success: false, message: error.message || 'Could not save 2FA phone.' });
    } finally {
        client.release();
    }
};

exports.updateOtpDeliveryMethod = async (req, res) => {
    try {
        const otpDeliveryMethod = String(req.body?.otpDeliveryMethod || 'email').trim() === 'phone' ? 'phone' : 'email';
        await ensureAccountSecuritySchema();
        const result = await pool.query(
            `UPDATE users
             SET otp_delivery_method = $1, updated_at = CURRENT_TIMESTAMP
             WHERE id = $2
             RETURNING id, email, two_factor_enabled, two_factor_phone_dial_code, two_factor_phone_number, otp_delivery_method`,
            [otpDeliveryMethod, req.user.id]
        );
        return res.status(200).json({ success: true, message: 'OTP delivery preference updated.', user: result.rows[0] });
    } catch (error) {
        console.error('Update OTP delivery method error:', error);
        return res.status(500).json({ success: false, message: 'Could not update OTP delivery preference.' });
    }
};
