...
const { saveUploadedFiles } = require('../modules/media');
const { buildManagedMediaSqlPredicate, classifyAdMediaSource, isManagedMediaUrl } = require('../modules/media/mediaAssetPolicy');
...
const classifyAdForSave = (row) => {
    return classifyAdMediaSource({
        activeLink: row.active_link || (row.edit_draft && (row.edit_draft.activeLink || row.edit_draft.active_link)) || '',
...
const RAW_PHOTO_VIDEO_UPLOAD_SQL = `
    ...
    AND ${buildManagedMediaSqlPredicate('a.media_preview')}
`;
...
const isRawUploadedPhotoVideoAd = (row) => {
    ...
    return isManagedMediaUrl(mediaPreview);
};
